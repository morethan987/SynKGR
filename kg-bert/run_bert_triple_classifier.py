...
from pytorch_pretrained_bert.file_utils import PYTORCH_PRETRAINED_BERT_CACHE, WEIGHTS_NAME, CONFIG_NAME  # type: ignore[import-untyped]
...
from pytorch_pretrained_bert.optimization import BertAdam, WarmupLinearSchedule  # type: ignore[import-untyped]
    ...
    def _create_examples(self, lines, set_type, data_dir):
        ...
        for (i, line) in enumerate(lines):
            ...
            if set_type == "dev" or set_type == "test":
                triple_label = line[3] if len(line) > 3 else "1"
                if triple_label == "1":
                    label = "1"
                else:
                    label = "0"
...
def main():
    ...
    if args.server_ip and args.server_port:
        ...
        import ptvsd  # type: ignore[import-untyped]
        print("Waiting for debugger attach")
        ptvsd.enable_attach(address=(args.server_ip, args.server_port), redirect_output=True)  # type: ignore[import-untyped]
        ptvsd.wait_for_attach()  # type: ignore[import-untyped]
    ...
    if args.do_train:
        ...
        if args.local_rank != -1:
            num_train_optimization_steps = num_train_optimization_steps // torch.distributed.get_world_size()  # type: ignore[attr-defined]
    ...
    if args.local_rank != -1:
        try:
            from apex.parallel import DistributedDataParallel as DDP  # type: ignore[import-untyped]
        except ImportError:
            raise ImportError("Please install apex from https://www.github.com/nvidia/apex to use distributed and fp16 training.")

        model = DDP(model)  # type: ignore[no-redef]
    elif n_gpu > 1:
    ...
    if args.fp16:
        try:
            from apex.optimizers import FP16_Optimizer  # type: ignore[import-untyped]
            from apex.optimizers import FusedAdam  # type: ignore[import-untyped]
        except ImportError:
            raise ImportError("Please install apex from https://www.github.com/nvidia/apex to use distributed and fp16 training.")
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):  # type: ignore[attr-defined]
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):  # type: ignore[attr-defined]
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):  # type: ignore[attr-defined]
